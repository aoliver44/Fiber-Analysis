...
head -1 count_reads.txt | cat - count_reads_tmp.txt > count_reads_clean.txt
...
while read line; do grep -m 1 ${line} genome_taxonomy.txt | awk -F '[\t]' '{print $4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$10}'; done < tmp1.txt > tmp2.txt
